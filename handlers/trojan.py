...
# === BAGIAN BELI TROJAN ===
@bot.callback_query_handler(func=lambda call: call.data == "buy_trojan")
def buy_trojan(call):
    ...
    msg = bot.send_message(call.message.chat.id, 
        f"<b>🐎 BELI TROJAN (Rp {price:,})</b>\n\nMasukkan <b>Username</b> yang diinginkan:", 
        parse_mode='HTML', reply_markup=m)
    
    bot.register_next_step_handler(msg, trojan_process)

def trojan_process(m):
    u = m.text.strip()
    ...
    status = bot.reply_to(m, "⏳ <b>Memproses Akun Trojan Premium...</b>", parse_mode='HTML')
    threading.Thread(target=trojan_execution, args=(m, u, uid, price, status)).start()

def trojan_execution(m, u, uid, price, status_msg):
    try:
        quota_gb = "100"      
        masa_aktif = "30"     
        limit_ip = 2          

        succ, info, res = create_trojan_user(u, quota_gb, masa_aktif)
        
        if succ:
            if str(uid) != str(ADMIN_ID): 
                try:
                    add_balance(uid, -price, f"Beli Trojan {u}")
                    try:
...
<code>{link_grpc}</code>
<code>☉——————————————————————————☉</code>
Format OpenClash : https://{domain}:81/trojan-{u}.txt
<code>☉——————————————————————————☉</code>
...
<code>☉——————————————————————————☉</code>"""
        ...
        print(f"CRITICAL ERROR TROJAN: {e}")
        try: bot.delete_message(m.chat.id, status_msg.message_id)
        except: pass
        bot.reply_to(m, "❌ Terjadi Kesalahan Sistem.")

# === CEK & DELETE TROJAN ===
def render_trojan_page(chat_id, message_id, page=0):
    try:
        cmd = f"{PATH_KYT}/bot-cek-tr" 
        if not os.path.exists(cmd): 
            bot.send_message(chat_id, "⚠️ Script bot-cek-tr tidak ditemukan!")
            return
        ...
        for line in raw.splitlines():
            if "|" in line:
                p = line.split("|")
                if len(p) >= 4:
                    users.append({"u": p[0].strip(), "ex": p[1].strip(), "us": p[2].strip(), "ip": p[3].strip()})
    except Exception as e: 
        print(f"Error Trojan: {e}")
        users = []
    ...
    msg = f"<b>🐎 LIST TROJAN</b>\nHal: {page+1}/{total} | Total: {len(users)}\n━━━━━━━━━━\n"
    m = telebot.types.InlineKeyboardMarkup()
    
    for u in cur:
        try: ip_count = int(u['ip'])
        except: ip_count = 0
            
        ic = "🟢" if ip_count > 0 else "⚪"
        msg += f"👤 <b>{u['u']}</b>\n📊 {u['us']} | 🔌 {ic} IP: {ip_count}\n📅 {u['ex']}\n➖➖➖➖\n"
        m.add(telebot.types.InlineKeyboardButton(f"🗑️ Hapus {u['u']}", callback_data=f"trj_del_{u['u']}_{page}"))
    
    nav = []
    if page > 0: nav.append(telebot.types.InlineKeyboardButton("⬅️", callback_data=f"trj_nav_{page-1}"))
    if page < total - 1: nav.append(telebot.types.InlineKeyboardButton("➡️", callback_data=f"trj_nav_{page+1}"))
    if nav: m.row(*nav)
...
@bot.callback_query_handler(func=lambda call: call.data == "check_trojan_vps")
def start_cek_trojan(call): 
    if str(call.from_user.id) == str(ADMIN_ID): 
        bot.answer_callback_query(call.id, "Loading Trojan...")
        render_trojan_page(call.message.chat.id, call.message.message_id, 0)

@bot.callback_query_handler(func=lambda call: call.data.startswith("trj_nav_"))
def nav_trojan(call):
    if str(call.from_user.id) == str(ADMIN_ID): 
        render_trojan_page(call.message.chat.id, call.message.message_id, int(call.data.split("_")[2]))

@bot.callback_query_handler(func=lambda call: call.data.startswith("trj_del_"))
def del_trojan(call):
    ...
    try:
        ...
        render_trojan_page(call.message.chat.id, call.message.message_id, page)
    except Exception as e:
